...
fprintf('\nOriginal vector bbar: [');
fprintf('%g, ', bbar(1:end-1));
fprintf('%g]', bbar(end));

% Initialize x = (x_beta) = (x_beta)
%                ( x_nu )   (   0  )
x(beta,1) = bbar;
x(ny,1) = zeros(length(ny),1);

fprintf('\nVector x from iter = 0: [');
fprintf('%g, ', x(1:end-1));
fprintf('%g]', x(end));
...
% Dantzig rule
  ...
  if rnymin < -fuzz
    ...
    if iter == 0
        fprintf('\nVector Abark: [');
        fprintf('%g, ', Abark(1:end-1));
        fprintf('%g]', Abark(end));

        fprintf('\nVector Abarkpos: [');
        fprintf('%g, ', Abarkpos(1:end-1));
        fprintf('%g]\n', Abarkpos(end));
    end
    
    % If column has a positive element, we pivot 
    ...
    else % Otherwise, unbounded solution and we stop.
    ...
    % Interchange elements of basis and non-basis
    betap = beta(p);
    nyq = ny(q);        
    beta(p) = nyq;
    ny(q) = betap;
